...
void	Commands::cmd_list(vector<Client*> &repertory, Client *client)
{
	...
	while (it != channels.end())
	{
		...
		{
			str = chan->getName() + " " + to_string(chan->getUserCount()) + " :" + chan->getTopic();
			client->numeric_reply("322", str);
		}
		it++;
	}
	client->numeric_reply("323", ":end of /LIST");
}
